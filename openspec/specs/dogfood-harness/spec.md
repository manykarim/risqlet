# dogfood-harness Specification

## Purpose
TBD - created by archiving change add-dogfooding-experiments. Update Purpose after archive.
## Requirements
### Requirement: Repeatable target preparation and cleanup
The harness SHALL prepare a target repo (install skills into the target's `.claude/skills/`, make `risqlet` invocable) and SHALL restore it afterwards: experiment artifacts (`.risqlet/`, installed skills) are removed, and the target's `git status --porcelain` after cleanup SHALL match the recorded baseline, with any residue reported loudly rather than silently left.
...
Each experiment run SHALL invoke the Claude Code CLI headlessly with a stored prompt file, a wall-clock timeout, restricted allowed tools (read/search, risqlet commands, read-only git, writes scoped to the register), and captured full output saved under `docs/experiments/<target>/<experiment>/`.
