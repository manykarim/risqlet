# Spec: scoring-policies

## ADDED Requirements

### Requirement: Policies are data packs
Scoring policies SHALL be defined entirely in YAML data packs declaring ordinal `factors` (name, min, max) and `derived` fields computed either by a `product` formula or a top-down first-match `lookup` band table. The engine SHALL load packaged policies (`sod-ap-v1`, `li-v1`) and user-supplied packs from `.qrisk/policies/`, validating pack structure before use. Adding a policy SHALL NOT require code changes.
...
- **WHEN** `.qrisk/policies/custom-v1.yaml` exists and config selects `custom-v1`
