...
`qrisk init` SHALL create the `.qrisk/` layout with a commented starter `config.yaml`, empty `register/`, and empty `events.jsonl`. It SHALL refuse to overwrite an existing non-empty `.qrisk/` directory.
...
`qrisk score` SHALL compute derived fields for one risk (`qrisk score R-0001`) or all scored risks (`--all`) under each score set's declared policy, writing results back into the risk files while preserving YAML comments and key order. It SHALL never create or modify factor values.

#### Scenario: Batch scoring
- **WHEN** `qrisk score --all` runs on three risks with valid score sets
...
- **WHEN** `qrisk validate --dir /elsewhere/project` runs
