...
`qrisk mcp` SHALL run a stdio MCP server exposing at most 10 tools, every tool taking an explicit `project_dir` and holding no server-side session state. The `mcp` dependency SHALL be an optional extra; running `qrisk mcp` without it SHALL print an actionable install hint and exit 1. The server SHALL perform no LLM inference.
...
- **WHEN** `qrisk mcp` runs without the mcp package installed
- **THEN** it exits 1 telling the user to install qrisk[mcp]
...
- **THEN** its report equals `qrisk validate --json` output for the same state
