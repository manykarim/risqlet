...
When a risk carries two or more score sets for the active policy, `qrisk score` SHALL compute `disagreement` as the mean over factors of the normalized value spread (0–1, rounded to 2 decimals) with per-factor detail, write it to the risk, and remove it when fewer than two qualifying score sets remain. `qrisk validate` SHALL recompute disagreement and fail on mismatch. Score sets for other policies SHALL be ignored by the computation.
...
`qrisk status` SHALL include a pending hint naming risks in proposed, reviewed, or accepted status whose disagreement value exceeds 0.25.
